...
export class TrainingPlanCardComponent implements OnInit {
  ...
  onDelete() {
    this.confirmationDialogService
      ...
      .then((confirmed) => {
        ...
        this.trainingStorage.onRemoveTrainingPlan(parameters);
  ...
  onToggleMain() {
    const parameters = [
      ...
      !this.TrainingPlan.isMain,
    ];
    this.TrainingPlan.isMain = !this.TrainingPlan.isMain;

    this.trainingPlanService
      .ToggleMain(parameters)
      .subscribe((x) => console.warn(x));
  }

  onTrainingPlanSelected() {
    this.router.navigate(["add-training-plan/", this.TrainingPlan.id, true], {
